...
## 퀵 정렬의 캐시 히트율이 높은 이유
퀵 정렬이 캐시 히트율이 높은 주된 이유는 공간적 지역성 덕분이다.

### 인접한 데이터에 접근
퀵 정렬은 배열의 특정 범위(서브배열)를 반복적으로 분할하고 처리한다.

이 과정에서 연속된 메모리 영역에 접근하는 경우가 많다.

피벗을 기준으로 왼쪽과 오른쪽으로 분할하면, 이 두 영역은 물리적으로 배열 내에서 서로 인접한 데이터이다.

분할된 하위 배열에서도 같은 방식으로 재귀적으로 처리하므로 계속해서 연속된 메모리를 사용하게 된다.

### 재귀 호출의 지역성

퀵 정렬은 재귀적으로 호출되지만 각 호출 스택의 작업 영역(서브배열)은 원래 배열의 작은 부분이다.

즉, 각 단계에서 다루는 데이터는 메모리 상에서 서로 가깝게 위치해 있다.

이러한 특징이 캐시 메모리에서 효율적으로 동작하게 만든다.

### 캐시 라인 활용

캐시 메모리는 데이터를 캐시 라인(Cache Line) 단위로 가져온다.


캐시 라인은 보통 64바이트 정도의 크기인데, 한 번에 연속된 데이터를 가져온다.


퀵 정렬은 인접한 데이터를 순회하기 때문에 한 번 캐시에 데이터가 로드되면 다음 데이터도 함께 가져와진다.
이로 인해 캐시 히트율이 높아진다.
